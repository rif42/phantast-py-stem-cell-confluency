...
from PyQt6.QtWidgets import (
    ...
    QSpacerItem, QSizePolicy, QFrame, QScrollArea, QMenu, QSplitter
)
...
class PipelineNodeWidget(QFrame):
    ...
    def __init__(self, node_data, is_selected=False):
        ...
        self.is_draggable = self.ntype not in ['input', 'algorithm'] # 'algorithm' is output here
    ...
    def init_ui(self):
        ...
        icon_frame.setStyleSheet("background-color: #2D3336; border-radius: 4px; border: none;")
    ...
    def mouseMoveEvent(self, event):
        ...
        if (event.pos() - self.drag_start_position).manhattanLength() < 5: # QApplication.startDragDistance()
            return
...
class PipelineConstructionWidget(QWidget):
    ...
    def create_left_panel(self):
        ...
        self.add_menu.setStyleSheet("""
            QMenu { background-color: #1E2224; border: 1px solid #2D3336; color: #E8EAED; }
            QMenu::item:selected { background-color: #2D3336; }
        """)
        ...
        self.add_menu.triggered.connect(lambda action: self.add_step.emit(action.data()))
    ...
    def render_nodes(self):
        ...
        for i, node in enumerate(nodes):
            ...
            if is_selected:
                ...
                if is_selected: border_color = "#60A5FA"
            elif ntype == 'algorithm':
                border_color = "#E8A317" # orange for output
                if is_selected: border_color = "#FCD34D"
            ...
            if i < len(nodes) - 1:
                ...
                line_layout.setContentsMargins(36, 0, 0, 0) # align with left side of node
                line = QFrame()
                line.setFrameShape(QFrame.Shape.VLine)
                line.setStyleSheet("color: #00B884; border-left: 1.5px solid #00B884; max-width: 1px;")
        ...
        for i in range(self.nodes_inner_layout.count()):
            ...
            if isinstance(w, PipelineNodeWidget):
                ...
                if drop_y < w.y() + (w.height() / 2):
                    ...
                    new_idx = next((j for j, n in enumerate(nodes) if n.get('id') == target_node_id), new_idx)
                    break
                else:
                    # Otherwise, assume it goes after this widget
                    new_idx = next((j for j, n in enumerate(nodes) if n.get('id') == w.node_id), new_idx) + 1
    ...
    def update_right_panel(self):
        ...
        active_node = next((n for n in self.pipeline.get('nodes', []) if n.get('id') == self.active_node_id), None)
        ...
        if active_node:
            ...
            if help_text:
                ...
                help_btn.clicked.connect(lambda _, t=help_text, b=help_btn: self.show_help_tooltip(t, b))
                title_layout.addWidget(help_btn, alignment=Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
            ...
            if active_node.get('name') == 'CLAHE':
                self.add_spinbox_row("Clip Limit", "2.0", "Threshold for contrast limiting.")
    ...
    def show_help_tooltip(self, text, button):
        if hasattr(self, '_current_tooltip') and getattr(self, '_current_tooltip', None):
    ...
    def create_canvas_area(self):
        ...
        self.image_label = QLabel("Central Canvas area.\nThis would show the image with processing applied.")
        self.image_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.image_label.setStyleSheet("color: #9AA0A6; font-size: 14px;")
